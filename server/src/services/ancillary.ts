 ...
 *
 * Now each such function is an *invocation* listed below. The admin pins any
 * of them to a model from the roster; an invocation without a pin runs on the
 * household's ancillary model, which is forced to exist (db.ts backfills it
 * from the chat default). Resolution never returns nothing.
 *

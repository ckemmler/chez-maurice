...
rm -rf "$DATA"
mkdir -p "$DATA" "$GARDENS"

cd "$REPO/server"
echo "→ Seeding…"
MAURICE_DATA_DIR="$DATA" MAURICE_GARDENS_DIR="$GARDENS" "$BUN" run scripts/seed-demo.ts

if [[ "${1:-}" == "--seed" ]]; then
  echo "→ Seed-only. Start later with:"
  echo "    cd server && MAURICE_DATA_DIR=$DATA MAURICE_GARDENS_DIR=$GARDENS PORT=$PORT $BUN run index.ts"
  exit 0
fi

echo "→ Starting demo server on http://localhost:$PORT  (Ctrl-C to stop)"
exec env MAURICE_DATA_DIR="$DATA" MAURICE_GARDENS_DIR="$GARDENS" PORT="$PORT" "$BUN" run index.ts

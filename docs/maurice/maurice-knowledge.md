...
**Reachable at last, and in an order, 20 September 2026.** The corpus was an *experimental* family, off by default, which meant it was in none of the owner's conversations: a question landing squarely in one of his domains could not be followed up although the index held the answer. It is now **always on**, with the two guards that being always on requires — never in a room, and only its reading tools — both described in [[maurice-tools]]. The second half of the change is an **order of search**, because the corpus itself has none: the only ranking is the cosine score, every source mixed, and conversations outnumber everything else by a factor of sixty in the owner's index (88 726 chunks against 1 408 notes). Measured on his own store, *"Où en suis-je dans ma pratique du violon ?"* unfiltered returns four conversations and not one note, while a garden note answers it. So the everyday prompt walks the order itself, one filtered search per step (`corpusNotice` in `services/claude.ts`): **the garden first** (`source_type` note, fiche, card, fragment) — what the member decided was worth keeping on a subject, which is the promise Maurice makes them — **then what they have already said** (`conversation`), **then what they only read or gathered** (`book`, `dossier`, `thought`). The domain briefs are the entry point: they say what matters, the corpus holds what was actually written (see [[maurice-domains]]). `corpus__search`'s own description was corrected at the same time — it listed neither `conversation` nor `card` among the `source_type` values, while conversations are the majority of the index.

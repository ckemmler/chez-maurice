...
- **Maurice comes back in the mail conversation** (lot 5, the night of 26–27 September 2026): once the documents pass wrote something, a message of his — rendered by the server in the member's language, no model — says how many fiches and digests went into the garden, gives the hub's path ("My mail") and the notes' titles. What he cannot do yet: answer a question about a note's source by clicking — `email__get_by_id` lets him read the message by the id the note carries, in the conversation. Tested on the owner's real thread: on "Oui, lis-les." the model called the tool once (70 ms) and answered in two rounds; `job_id` null on the turn's ledger row, the reading's own rows carry it.

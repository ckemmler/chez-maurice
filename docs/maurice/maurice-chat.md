...
- **The mail conversation's yes now triggers the reading** (lot 4, the same night) but not yet what Maurice promised: the passes leave sealed readings in the member's store and nothing comes back into the conversation until lot 5 writes the documents. Tested on the owner's real thread: on "Oui, lis-les." the model called the tool once (70 ms) and answered in two rounds; `job_id` null on the turn's ledger row, the reading's own rows carry it.

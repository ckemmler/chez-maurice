...
- **The first real documents run** (26 September 2026, the owner's box): 17 fiches and 6 digests for 0.08 € — two of the fiches were on the owner himself (writing from another address of his) and eight on services (MongoDB, OVH, Sanity, Medium…) the light pass had kept. Fixed the same night: a counterpart whose display name is the member's is the member; the writer answers `is_person: false` for a company, a shop, a platform or a team writing notices, and that key is recorded as declined and never asked again; the hub is recomputed at every run from what is on disk and rewritten only when it changed. The twelve notes were deleted by hand; seven fiches and five digests stand.

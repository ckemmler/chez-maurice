...
**Scaleway** caches automatically, per project, with no parameter (its FAQ promises 50–90% hits on recurring prefixes, not guaranteed, at a discounted rate). Whether the cached count comes back in the usage field is unverified — no key on this instance yet — and its price sheet lists a cached-input price for DeepSeek V4 Flash only; `pricing.ts` bills a cached token at full price for the others until the sheet says otherwise.
...
- **Scaleway's cache is unmeasured.** The default models for sold instances will be Scaleway's; the loop reads `prompt_tokens_details.cached_tokens` as for OpenAI, but nobody has seen a Scaleway response carry it. First thing to check once a key is in the household.

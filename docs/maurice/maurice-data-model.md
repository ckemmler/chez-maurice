...
| `mail_accounts` | `id`, `member_id` (cascade), `address` (unique per member), `name`, `provider`, `host`, `port`, `security`, `username` (all optional — the `email` tool guesses them from the domain), `secret`, `state` ∈ `unchecked`/`ok`/`error`, `last_error`, `checked_at`, `created_at`, `updated_at` | A member's mailbox, added by the member themselves (25 September 2026, `services/mailAccounts.ts`). `secret` is the password sealed with AES-256-GCM (`v1:` + base64 of IV, tag, ciphertext) under the **household key**: `MAURICE_SECRET_KEY` (32 bytes, hex or base64) when set, else `secret.key` in the app dir, created on first use, mode 0600. A copy of `maurice.db` without the key carries no usable password; the household archive carries the key. No route returns `secret`; only the loopback route the tool reads decrypts it. |
...
| Table | Key columns | Notes |
|---|---|---|
| `conversations` | `id`, `user_id` (owner), `title`, `maurice_id` (bound persona, null = everyday), `origin` (null, `'anthropic'` or `'chatgpt'` for imports), `imported_at` (nullable; the wall-clock of the import, 19 September 2026), `tool_families` (JSON, nullable), `context_from` (nullable message id), `opened_by ∈ {member,maurice}` | `origin='anthropic'` marks a thread imported from a Claude export → the sidebar badge. `context_from` is where the model's window starts once the history outgrew it — see [[maurice-server]]. `opened_by` (19 September 2026) is who started the thread: `member`, or `maurice` for a conversation he opened himself — the operator's hand (`POST /api/admin/conversations/open`) or, since the same evening, the night that proposes domains (`services/domainMapping.ts`); such a conversation is where the `domains__*` tools exist. A conversation Maurice opened is never read by the mapping. `imported_at` is what lets the briefs' night read an imported conversation whose messages predate its `read_until`: set by the corpus importer (`chat_import.py`), compared with the brief's `updated_at` (`services/domainBriefs.ts`). |
...
| Table | Key columns | Notes |
|---|---|---|
| `maurices` | `id`, `name`, `kind` (`domain` — the default, NULL reads as such — or `companion`, a reading companion; sorted once at first start for the rows that predate it, 19 September 2026), `hat` and `palette` (the personas' leftovers: nothing writes or reads them), `model` (nullable), `temp` (creativity), `thinking` (nullable 0/1), `tagline`, `prompt`, `context_json` (frozen spec), `tool_families` (nullable), `created_by` | A [[maurice-personas-hats|specialized Maurice]]. `context_json` is the persona's **baked-in** context. `thinking` is its reasoning choice for a model whose roster entry is `optional`: NULL leaves the provider's default, 1 asks for the phase, 0 skips it — except on Z.ai, where NULL is sent as `reasoning_effort: low` (24 September 2026): the phase cannot be turned off there and the provider's own default is a minute of reasoning a turn, so a seeded domain — every one of which carries NULL — was taking two minutes to answer on GLM-5.3-Flash. |
| `domain_briefs` | `maurice_id` + `member_id` (PK, both cascade), `text`, `updated_at`, `sources_json`, `read_until`, `model` | The **brief** of a domain (19 September 2026): `maurices` is the table of domains seen from the other side — a part of a member's life that Maurice follows — and this is the short text he keeps on it, rewritten at night from the previous one and the conversations that touched the domain since (`services/domainBriefs.ts`, see [[maurice-server]]). `sources_json` lists the conversations the last rewrite read; `read_until` is the `created_at` of the newest message it saw, and the next rewrite reads only what came after — a night with nothing new writes nothing. `model` is what wrote it — `member` when the member corrected it in the app, and the next rewrite then starts from their words. Read by the everyday Maurice's prompt (the member's own conversations only, 3 000 tokens for all of them) and by the app's domain page; see [[maurice-domains]]. |
| `domain_proposals` | `id`, `member_id` (cascade), `name`, `summary`, `conversation_ids_json`, `state ∈ {proposed, adopted, dismissed, expired, superseded}`, `presented` (0/1), `conversation_id` (nullable, set null on delete), `maurice_id` (nullable, set null on delete), `stats_json`, `created_at`, `updated_at` | A **domain proposal** (19 September 2026, P2-B): what the night's mapping found in a member's unattached conversations — the name and paragraph the night model wrote, the conversations that justify it, the numbers behind the verdict in `stats_json` (size, months active, first and last day, conversations in 90 and 365 days, imported, cohesion, `verdict` alive or lived, the model's `is_domain` and `split_hint`, `origin` mapping / split / model_split / member / merge, the night). `proposed` until the member speaks; `adopted` once `domains__adopt` made a domain of it (`maurice_id`); `dismissed` when refused — its conversations are never mapped again; `expired` after forty-two days unanswered; `superseded` when merged into or split out of another. `presented` marks the three the opening message shows; `conversation_id` is the conversation Maurice opened to carry them, the only place the tools exist (`services/domainProposals.ts`, see [[maurice-domains]]). |
| `maurice_access` | `maurice_id` + `member_id` (PK) | Which members may use a domain or companion (also how guests get the ones an admin grants them). |
...
| `ancillary_advised` | `invocation` (PK), `at` | Which invocations were ever offered their pin on this household (19 September 2026). The seed runs once and the refresh revises only pins that exist, so a function added to the catalogue afterwards had none; at start, an invocation with no row and no pin is pinned to its advice and marked, a pin the admin deleted keeps its row and is never re-created. See [[maurice-server]]. |
...
- **The ledger has a spender that is not a member (19 September 2026).** `user_id = 'system'` marks what Maurice spends on nobody's turn — the domain briefs at night, later the mapping. It is not a row in `users`, deliberately: the column has no foreign key, and the night is the household's cost, not anyone's. Its own cap is `households.spend_cap_system_daily_usd`; a member's cap never touches it and it never touches a member's. Still not in the ledger: the ancillary calls made *on a member's behalf* — conversation and article summaries, flashcards, signals — which cost money and are charged to nobody.
